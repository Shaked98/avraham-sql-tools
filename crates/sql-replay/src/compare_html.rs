...
fn meta_rows(out: &mut String, b: &RunMeta, c: &RunMeta) {
    ...
    let rows: [(&str, String, String); 10] = [
        ("run file", b.file.clone(), c.file.clone()),
        (
            "target server version",
            b.target_server_version.clone(),
            c.target_server_version.clone(),
        ),
        ("target url", b.target_url.clone(), c.target_url.clone()),
        (
            "capture file",
...
pub fn render_html(r: &CompareReport) -> String {
    ...
    let title = format!(
        ...
        r.baseline.target_server_version, r.candidate.target_server_version
    );
    let _ = write!(
        out,
        r#"<!doctype html>
...
"#,
        title = esc(&title),
        bver = esc(&r.baseline.target_server_version),
        bfile = esc(&r.baseline.file),
        cver = esc(&r.candidate.target_server_version),
